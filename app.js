...
function setModuleState(module, state, label) {
  ...
  module.classList.remove("scanning", "complete", "warning", "flagged");

  if (state) module.classList.add(state);
...
function setScanProgress(percent, phase) {
  if (el.scanPercent) el.scanPercent.textContent = `${percent}%`;
  if (el.scanProgress) el.scanProgress.style.width = `${percent}%`;
  if (el.scanPhase) el.scanPhase.textContent = phase;
}
...
const diagnosticRules = {
  overheat: {
    ...
    confidence: 86,
    urgency: "Critical",
    safe: "No. Stop safely, switch the engine off and allow it to cool.",
    mot: "Possible MOT issue if coolant leaks, warning lights or emissions problems are present.",
    explanation: "The cooling system is showing overheating behaviour linked to coolant flow or thermal regulation issues.",
    causes: [
      "Low coolant level",
      "Coolant leak",
  ...
  misfire: {
    ...
    confidence: 88,
    urgency: "High",
    safe: "Limited. Avoid driving far because misfires can damage the catalytic converter.",
    mot: "Likely MOT issue if emissions are affected or the engine warning light is on.",
    explanation: "Engine misfire behaviour detected. Ignition and fuel systems require inspection.",
    causes: [
  ...
  no_start: {
    ...
    explanation: "Vehicle starting failure linked to battery, starter, immobiliser or fuel delivery.",
    causes: [
      "Weak battery",
  ...
  battery: {
    ...
    confidence: 83,
    urgency: "Medium",
    safe: "Short driving may be possible, but the vehicle may stop once battery power drops.",
    mot: "May affect MOT if electrical systems, lamps or warning lights are not operating correctly.",
    explanation: "Charging system irregularities detected. Alternator or battery performance issue likely.",
    causes: [
      "Alternator fault",
      "Weak battery",
  ...
  rough_idle: {
    ...
    confidence: 74,
    urgency: "Medium",
    safe: "Short driving may be possible if the vehicle is stable, but avoid driving if it stalls or shakes heavily.",
    mot: "May affect MOT if emissions are high, idle is unstable or warning lights are present.",
    explanation: "Unstable idle behaviour detected. Air/fuel balance or idle control issue possible.",
    causes: [
      "Dirty throttle body",
      "Vacuum leak",
      "MAF sensor fault",
...
function generateDiagnosis(key) {
  ...
  return {
    ...
    causes: rule.causes,
    checks: rule.checks,
    steps: rule.checks.map((s, i) => `${i + 1}. ${s}`).join("\n")
  };
}
...
function renderSymptomResult() {
  ...
  if (!value) {
    if (el.symptomResult) {
      ...
      el.symptomResult.textContent = "Select a symptom to view possible causes, urgency, MOT impact, safe-to-drive advice and next checks.";
    }
    return;
  }
  ...
  if (!diagnosis || !el.symptomResult) return;
  ...
  el.symptomResult.innerHTML = `
    <div class="symptom-card">
      ...
      <div class="symptom-meta">
        <span>Confidence ${escapeHtml(diagnosis.confidence)}%</span>
        <span>${escapeHtml(diagnosis.urgency)} urgency</span>
      </div>
      ...
      <div class="symptom-info-grid">
        <div class="symptom-info">
          <h4>Likely Causes</h4>
          <ul>${diagnosis.causes.map(c => `<li>${escapeHtml(c)}</li>`).join("")}</ul>
        </div>
        ...
        <div class="symptom-info">
          <h4>Next Checks</h4>
          <ul>${diagnosis.checks.map(c => `<li>${escapeHtml(c)}</li>`).join("")}</ul>
        </div>
      </div>
    </div>
  `;

  if (el.fault) el.fault.value = diagnosticKey;
}
...
function renderWarningLights() {
  ...
  if (!filtered.length) {
    el.warningLightResults.innerHTML = `<div class="no-warning-results">No warning lights found.</div>`;
    return;
  }
...
function lookupCode() {
  ...
  if (!code || !el.codeOut) return;

  const result = obdCodes[code];
  el.codeOut.textContent = result || "Unknown fault code";
}
...
function calculateInvoice() {
  ...
  setText(el.invoiceStatus, lastInvoice.paymentStatus);
}

async function runAI() {
  const selected = el.fault?.value || "misfire";
  const diagnosis = generateDiagnosis(selected);
  ...
  setLoading(el.runAiBtn, true, "Scanning...");
  setText(el.result, "Scanning...");
  setText(el.confidence, "--");
  setText(el.aiExplain, "");
  setText(el.steps, "");

  if (el.scanState) el.scanState.textContent = "SCANNING";

  setScanProgress(6, "Initialising vehicle session");
  addFeedLine(`Vehicle session ${diagnosis.sessionId} started.`);

  await wait(600);

  setScanProgress(18, "Checking engine control systems");
  setModuleState(el.moduleEngine, "scanning", "Scanning");
  addFeedLine("Reading engine control module response...");

  await wait(700);

  setModuleState(el.moduleEngine, "complete", "Complete");
  setScanProgress(34, "Analysing ignition behaviour");
  setModuleState(el.moduleIgnition, "scanning", "Analysing");
  addFeedLine("Comparing ignition patterns against known faults...");

  await wait(800);

  if (selected === "misfire") {
    setModuleState(el.moduleIgnition, "warning", "Warning");
    addFeedLine("Ignition instability detected.");
  } else {
    setModuleState(el.moduleIgnition, "complete", "Complete");
  }

  setScanProgress(57, "Checking fuel delivery systems");
  setModuleState(el.moduleFuel, "scanning", "Analysing");
  addFeedLine("Checking fuel trim and injector behaviour...");

  await wait(850);

  setModuleState(el.moduleFuel, "complete", "Complete");
  setScanProgress(78, "Reviewing emissions and MOT risk");
  setModuleState(el.moduleEmissions, "scanning", "Reviewing");
  addFeedLine("Calculating MOT and emissions impact...");

  await wait(900);

  setModuleState(el.moduleEmissions, "flagged", "Flagged");
  setScanProgress(92, "Generating workshop report");
  addFeedLine(`Primary fault group identified: ${diagnosis.title}`);
  addFeedLine(`Confidence score updated: ${diagnosis.confidence}%`);

  await wait(1000);

  setText(el.result, diagnosis.title);
  setText(el.confidence, `${diagnosis.confidence}%`);
  setText(el.aiExplain, diagnosis.explanation);
  setText(el.steps, diagnosis.steps);

  setScanProgress(100, "Diagnostic analysis complete");

  if (el.scanState) el.scanState.textContent = "COMPLETE";

  addFeedLine("Recommended diagnostic workflow generated.");
  addFeedLine("Report ready to save.");
...
async function saveCurrentReport() {
  ...
  try {
    ...
    const payload = {
      ...
      vehicleMake: el.vehicleMake?.value || "",
      vehicleModel: el.vehicleModel?.value || "",
      vehicleReg: el.vehicleReg?.value || "",
      vehicleMileage: el.vehicleMileage?.value || "",
      serviceNotes: el.serviceNotes?.value || "",
      ...lastReport,
      ...(lastInvoice || {})
    };
...
function renderReports(list) {
  ...
  list.forEach(report => {
    ...
    card.innerHTML = `
      ...
      <div class="report-meta"><strong>Confidence:</strong> ${escapeHtml(report.confidence || 0)}%</div>
      <div class="report-meta"><strong>Invoice Total:</strong> ${money(report.grandTotal || 0)}</div>

      <pre>${escapeHtml(report.explanation || "")}</pre>

      <a class="primary-link" href="report.html?id=${encodeURIComponent(report.id)}" target="_blank">
...
async function loadReports() {
  ...
  try {
    ...
    setMessage(el.reportMessage, err.message || "Failed to load reports.", "error");
  }
...
function updateStats() {
  ...
  allReports.forEach(report => {
    totalRevenue += Number(report.grandTotal || 0);
    confidenceTotal += Number(report.confidence || 0);

    if (report.customerName) customerSet.add(report.customerName);
    if (report.vehicleReg) vehicleSet.add(report.vehicleReg);

    const fault = report.title || report.fault || "Unknown";
  ...
  setText(el.totalReports, totalReports);
  setText(el.totalReportsMirror, totalReports);
  setText(el.totalCustomers, customerSet.size);
  setText(el.totalVehicles, vehicleSet.size);
  setText(el.totalRevenue, money(totalRevenue));
  setText(el.totalRevenueMirror, money(totalRevenue));
  setText(el.avgConfidence, `${avg}%`);
  setText(el.topFault, topFault);
}
...
async function login() {
  ...
  try {
    ...
    setMessage(el.authMessage, "Logged in.", "success");
  } catch (err) {
    setMessage(el.authMessage, err.message || "Login failed.", "error");
  }
...
async function signup() {
  ...
  try {
    ...
    setMessage(el.authMessage, err.message || "Signup failed.", "error");
  }
...
if (el.lookupBtn) el.lookupBtn.onclick = lookupCode;
...
if (el.saveReportBtn) el.saveReportBtn.onclick = saveCurrentReport;
